...
const _updated = async (key: IDBValidKey) => {
...
function set<K extends keyof DBAccessors>(
  key: K,
  value: DBAccessors[K],
  sync?: boolean,
): Promise<void>;
...
function update<K extends keyof DBAccessors>(
  key: K,
  f: (val: DBAccessors[K]) => DBAccessors[K],
