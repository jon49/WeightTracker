...
function getWakeUp(bedtime: string | undefined, sleep: number | undefined) {
  return !bedtime
    ...
    : !sleep
      ? html`
          <button id="wake-up" class="h-fit-content self-center" formaction="?handler=wakeUp">Wake Up</button>
        `
      : html`
...
function getBedtime(bedtime: string | undefined) {
  if (!bedtime) {
    return html`
      <button class="w-100" formaction="?handler=startSleep">Bedtime</button>
    `;
  }
  return html`
  ...
  async wakeUp({ data }) {
    ...
    if (o.bedtime) {
      ...
      let wakeUpDateTime = new Date(
        `1970-01-0${+(+o.bedtime.slice(0, 2) > +wakeUpTime.slice(0, 2)) + 1}T${wakeUpTime}`,
      );
