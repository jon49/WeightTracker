...
subscribe("create-chart", async ({ el }) => {
    if (!el) return
...
async function histogram() {
    ...
    if (!startDate) return
    const rawValues: (DB.WeightData|undefined)[] = (await getMany(dateFill(new Date(startDate), new Date())))
    const values = {}
